 ...
 */
export function useDistributedInference(config) {
    ...
    const generate = useCallback(async (request) => {
        ...
        try {
            ...
            const result = {
                ...
                source: (routingDecision.primary === 'edge' ? 'edge' : 'local'),
 ...
 */
export function useStreamingInference(config) {
    const [state, setState] = useState({
        isStreaming: false,
        tokens: [],
        text: '',
    ...
    const tokenTimestampsRef = useRef([]);
    /**
     * Stream inference
     */
    const stream = useCallback(async function* (request) {
        setState((s) => ({
            ...s,
            isStreaming: true,
            tokens: [],
        ...
        try {
            ...
            for (const word of words) {
                ...
                const recentTokens = tokenTimestampsRef.current.filter((t) => Date.now() - t < 1000);
...
export function useBudget() {
     ...
     */
    const checkBudget = useCallback((modelId, prompt, maxOutputTokens) => {
        // This would use actual TokenBudgetManager
        const estimatedTokens = Math.ceil(prompt.length / 4) + (maxOutputTokens || 1024);
        if (state.remaining.tokens !== null &&
     ...
     * Record usage
     */
    const recordUsage = useCallback((modelId, inputTokens, outputTokens) => {
        const record = {
            timestamp: Date.now(),
            modelId,
            inputTokens,
     ...
     * Set budget limits
     */
    const setBudgetLimits = useCallback((limits) => {
        setState((s) => ({
            ...s,
            remaining: {
...
export function useOfflineStatus() {
     ...
     */
    const cacheModel = useCallback(async (modelId) => {
     ...
     */
    const isModelCached = useCallback((modelId) => {
 ...
 */
export function useInferenceMetrics(options = {}) {
     ...
     * Record a metric
     */
    const recordMetric = useCallback((name, value, labels = {}) => {
        const metric = {
            name,
            type: 'gauge',
            value,
 ...
 */
export function useEdgeworkInference(config) {
